<Serializable()> _
Public Class PrintZelenInfo
  Inherits ReadOnlyBase(Of PrintZelenInfo)
    'Private opcii As Options = CType(Csla.ApplicationContext.LocalContext("objOpcii"), Options)
    Private _regIssuers As RegistrationIssuerList = CType(Csla.ApplicationContext.LocalContext("objRegistrationIssuerList"), RegistrationIssuerList)
    Private _currentOwner As PrintCustomerList = PrintCustomerList.EmptyList
    Private _pomTechnikalExamOrganizationInfo As TehnicalExamOrganizationsInfo = _
...
#End Region
  ...
  Public ReadOnly Property OrganizationName() As String
        Get
            If _organizationName <> String.Empty And _organizationName <> "" Then
                Return UCase(_organizationName)
            Else
                Return UCase(_pomTechnikalExamOrganizationInfo.OrganizationName)
            End If
        End Get
  End Property
  ...
  Public ReadOnly Property Station() As String
        Get
            If _station <> String.Empty And _organizationName <> "" Then
                Return UCase(_station)
            Else
                Return UCase(_pomTechnikalExamOrganizationInfo.Station)
            End If
  ...
  Friend Sub New(ByVal dr As SafeDataReader)
    ...
    _cityNameTehnicalOrganization = dr.GetString("CityNameTehnicalOrganization")
        ...
        Dim regIssuer As RegistrationIssuerInfo = _regIssuers.GetRegistrationIssuerInfo(_pomTechnikalExamOrganizationInfo.IdDefaultRegistrationIssuer)
