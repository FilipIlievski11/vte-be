...
| 13 | **Single-Trigger limitation**: v2 `PriceCatalog.Trigger` is one enum value per row; legacy has six `TrigerdBy*` flags on **PaymentCategories**. Verified safe 2026-07-30: the ONLY active multi-flag categories on the live legacy DB are the three „Технички преглед" ones (ids 11/53/1011, TE+IrregularTE), and v2 already carries BOTH sides as split rows (719 rules Trigger=1 + 48 rules Trigger=6, all 48 in those groups). Re-check the flags query if new legacy categories ever appear. |
