...
interface Country { id: number; name: string; iso2: string | null; iso3: string | null; citizenship: string | null; isActive: boolean }
interface EditForm { id: number | null; name: string; iso2: string; iso3: string; citizenship: string; isActive: boolean }

const { t } = useI18n()
const auth = useAuthStore()
const toast = useToast(); const { ask } = useConfirmDialog()

const rows = ref<Country[]>([])
const loading = ref(true)
const showDialog = ref(false)
const error = ref<string | null>(null)
const saving = ref(false)
const editing = ref<EditForm>({ id: null, name: '', iso2: '', iso3: '', citizenship: '', isActive: true })

// Single top-of-page search box (like Customers) — searches across name, short code, citizenship.
...
function newRow()  { editing.value = { id: null, name: '', iso2: '', iso3: '', citizenship: '', isActive: true }; error.value = null; showDialog.value = true }
function editRow(r: Country) {
  editing.value = { id: r.id, name: r.name, iso2: r.iso2 ?? '', iso3: r.iso3 ?? '', citizenship: r.citizenship ?? '', isActive: r.isActive }
...
async function save() {
  ...
  try {
    const payload = {
      ...
      iso2: editing.value.iso2 || null,
      iso3: editing.value.iso3 || null,
      citizenship: editing.value.citizenship || null,
      isActive: editing.value.isActive,
    }
    if (editing.value.id == null) await api.post('/countries', payload)
    else                          await api.put(`/countries/${editing.value.id}`, payload)
...
<template>
  ...
  <DataTable :value="rows" :loading="loading" stripedRows size="small"
             v-model:filters="filters" :globalFilterFields="['name','iso2','citizenship']"
    ...
    <Column field="iso2" :header="t('countries.shortName')" sortable style="width: 140px" />
    <Column field="citizenship" :header="t('countries.citizenship')" sortable style="width: 220px">
      <template #body="{ data }">
  ...
  <Dialog v-model:visible="showDialog" :header="editing.id == null ? t('common.add') : t('common.edit')" modal :style="{ width: '460px' }">
    <div class="field"><label>{{ t('countries.name') }} *</label><InputText v-model="editing.name" autofocus size="small" /></div>
    <div class="row">
      <div class="field" style="flex: 1"><label>{{ t('countries.shortName') }}</label><InputText v-model="editing.iso2" maxlength="5" size="small" /></div>
      <div class="field" style="flex: 1"><label>{{ t('countries.iso3') }}</label><InputText v-model="editing.iso3" maxlength="5" size="small" /></div>
    </div>
    <div class="field"><label>{{ t('countries.citizenship') }}</label><InputText v-model="editing.citizenship" maxlength="100" size="small" /></div>
