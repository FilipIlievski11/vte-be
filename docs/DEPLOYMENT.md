...
### Refreshing production data (done first on 2026-06-13)

Three steps, ~5 minutes total:

1. **Sync legacy → local** (laptop, local API running): press "Синхронизирај" on the Legacy Sync screen, or `POST /api/admin/legacy-sync` with an admin token. Pulls new clients/vehicles/registrations/relations/requests/proofs/tech-exams from the live legacy DB via the `VTEZVV_LIVE` linked server. (Sync only works locally — prod has `LegacySync__Enabled=false` by design.)
2. **Lift local → prod**: backup local VTE → gzip → scp to `/opt/vte/` → gunzip → `docker cp` into `vte-mssql-1` → stop `vte-api-1` → `RESTORE DATABASE VTE ... WITH REPLACE, MOVE` → start api.
3. **Re-secure admin**: the restore brings the LOCAL admin password — log in with it once and `POST /api/users/{adminId}/reset-password` to the strong password from `deploy/prod.secrets.local`.

> Note: the sync covers the operational register, NOT the historical `PaymentDocuments` bulk (that was a one-time snapshot migration; new v2 debts are generated by the app itself).
